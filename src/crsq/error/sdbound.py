...
class RunData:
    ...
    def set_psin_ks(self, psi1_ks):
        self._psin_ks = psi1_ks
...
def sdbound1(rd: RunData, alpha: float) -> float:
    ...
    g is an arbitrary real number. t is the total time for time evolution and N is the trotter round.
...
def sderror(rd: RunData) -> float:
    ...
    so is result from split operator. an is result from analytical calculation.
    """
